  ...
  <div>
    ...
    <div class="field" v-else>
      <div class="control has-icons-left">
        <div class="select is-rounded">
          <select v-model="bookNumber" v-on:change="setSelectedPassage()">
    ...
    <div class="table-container">
      <table class="table">
        <tbody>
          <tr>
            <td v-for="chapter in totalChapter" :key="chapter" class="p-1">
              <button class="button is-small is-rounded" v-on:click="getPassageChapter(chapter)">
                {{ chapter }}
              </button>
            </td>
          </tr>
    ...
    <div v-else>
      <ul>
        <li v-for="verse in passage.verses" :key="verse.book_number" class="mb-4 c-hover">
          <span class="has-text-weight-bold">({{ verse.verse }})</span>
...
export default {
  ...
  methods: {
    setSelectedPassage() {
      this.passage = []
      this.totalChapter = this.passageList[this.bookNumber-1].total_chapter
    },
    getPassageChapter(chapter) {
      this.skeleton = true
      this.passage = []
      APIService.getPassageChapter(this.passageList[this.bookNumber-1].abbreviation.replace(" ", "+"), chapter).then(
        (resp) => {
